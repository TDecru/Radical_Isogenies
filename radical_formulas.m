...
psi := DualIsogeny(phi);
assert psi(Qw)(PA) eq P;
newA := (1/4*A^3 + 41/64*A^2 + 75/1024*A + 11/16384)/(A^4 - 1/4*A^3 + 3/128*A^2 - 1/1024*A + 1/65536)*w^3 +
    (-1/2*A^3 - 15/32*A^2 - 15/512*A - 1/8192)/(A^4 - 1/4*A^3 + 3/128*A^2 - 1/1024*A + 1/65536)*w^2 +
    (11/16*A^3 + 75/256*A^2 + 41/4096*A + 1/65536)/(A^4 - 1/4*A^3 + 3/128*A^2 - 1/1024*A + 1/65536)*w +
    (-3/4*A^3 - 5/32*A^2 - 3/1024*A)/(A^4 - 1/4*A^3 + 3/128*A^2 - 1/1024*A + 1/65536);
newb := -newA; newc := 0;
...
psi := DualIsogeny(phi);
assert psi(Qw)(PA) eq P;
newA := (10*A - 30)/(A^2 - 11*A - 1)*w^4 + (5*A - 15)/(A^2 - 11*A - 1)*w^3 + (-15*A - 5)/(A^2 - 11*A - 1)*w^2 +
    (A^2 - 36*A - 1)/(A^2 - 11*A - 1)*w + (5*A^2 - 40*A)/(A^2 - 11*A - 1);
newb := newA; newc := newA;
Eiso := EllipticCurve([1-newc,-newb,-newb,0,0]);
assert jInvariant(EA) eq jInvariant(Eiso);
"Formulas verified for N equal", N;
